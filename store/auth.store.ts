...
const RolePermissions: Record<Role, string[]> = {
  ADMIN: ['*', 'view_dashboard', 'view_users', 'view_leads', 'view_tasks', 'view_reports', 'view_audit_log', 'view_settings'],
  MANAGER: ['view_dashboard', 'view_users', 'view_reports', 'view_audit_log'],
  AGENT: ['view_dashboard', 'view_leads', 'view_tasks'],
...
export const useAuthStore = create<AuthStore>((set) => ({
  ...
  setUser: (user) => {
    ...
    set({ user, permissions: [...rolePerms, ...extraPerms] });
  },
  ...
  removePermission: (permission) => {
    set((state) => ({ permissions: state.permissions.filter((p) => p !== permission) }));
  },
  ...
  checkAuth: async () => {
    ...
    try {
      ...
      set({ user, isAuthenticated: true, isLoading: false, permissions: [...rolePerms, ...extraPerms] });
    } catch {
      clearTokens();
      set({ user: null, isAuthenticated: false, isLoading: false, permissions: [] });
    }
